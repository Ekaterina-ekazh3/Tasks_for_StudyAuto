...
def define_winner_hand(scs: list) -> tuple:
    ...
    if len(hand_max) == 1:
        ...
        if max_score == 4:
            ...
            hand_max_without_three = copy.deepcopy(hand_max_without_suit)
            print(type(hand_max_without_three))
            for hand in hand_max_without_suit:
                value_of_three = hand[2]
                ind = hand_max_without_suit.index(hand)
                for i in range(3):
                    hand_max_without_three[ind].remove(value_of_three)

            winner_hand_without_suit = max(hand_max_without_three)
            windex = hand_max_without_three.index(winner_hand_without_suit)
            win_hand = hand_max[windex]
            return win_hand, max_score
